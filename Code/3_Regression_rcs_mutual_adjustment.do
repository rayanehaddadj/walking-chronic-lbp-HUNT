...
foreach var in walk_tot walk_mets {
	...
	// Define Plot Label and Additional Adjustement
	if ("`var'" == "walk_tot") {
		local adj walk_mets
		local letter "A"
	...
	// Variable Descriptives for Adjustment Variable
	quietly sum `adj', detail
	local p10_adj = r(p10)
	local p50_adj = r(p50)
	local p90_adj = r(p90)
	
	// Create Knots for Adjustment Variable
	quietly mkspline `adj'_spline = `adj', cubic nknots(3) knots(`p10_adj' `p50_adj' `p90_adj')
	
	// Multi-Adjusted RCS Poisson Regression	
	poisson lbp_t1 `var'_spline* age i.sex i.education i.income i.work_status i.smoking i.depression `adj'_spline*, vce(robust)
	...
	ylabel(`incr_y1' 1 "1" `incr_y2', tlcolor(black) labsize(small) format(%2.1f) glstyle(grid) glcolor(gray%15) axis(1)) ///
	xtitle("{bf: `label'}", size(small)) ///
	...
	graph save "### Your file path ###\Figures\Graph\\`var'_adj.gph", replace
	graph export "### Your file path ###\Figures\Graph\\`var'_adj.png", replace
	putdocx image "### Your file path ###\Figures\Graph\\`var'_adj.png"
}
...
// Combine Plots
graph combine "### Your file path ###\Figures\Graph\walk_tot_adj.gph" ///
"### Your file path ###\Figures\Graph\walk_mets_adj.gph", ///
col(1) xsize(4) graphregion(margin(zero))

// Export Plot in Png
graph export "### Your file path ###\Figures\Graph\duo_rcs_adj.png", replace

// Plot Title
putdocx text ("Figure X. Dose-Response Association of Volume of Daily Walking and Usual Walking Intensity at Baseline in 2017-2019 with Risk of Chronic Low Back Pain at Follow-Up in 2021-2023 After Mutal Adjustement"), bold

// Add Plot to Word Document
putdocx image "### Your file path ###\Figures\Graph\duo_rcs_adj.png"

// Plot Legend
putdocx text ("The left y-axis is a log scale with the shaded area representing 95% CIs. Model are adjusted for age, sex, education, income, employment status, smoking status and depression. Figure A is further adjusted for walking intensity. Figure B is further adjusted for walking volume. Reference is set at the 10th percentile of the distribution. Abbreviation: MET, Metabolic Equivalent of Task.")

// Save Word Document
putdocx save "### Your file path ###\Figures\Regression_rcs_plots_mutual_adjutsment.docx", replace
